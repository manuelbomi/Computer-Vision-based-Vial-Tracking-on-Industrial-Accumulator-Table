...
| [`dashboard/`](dashboard) | A React + TypeScript **web dashboard**: interactive disposition/radius charts and a sortable per-vial table, reading directly from `vial_analytics.py`'s CSV output. See [Web dashboard](#web-dashboard). |
...
[`dashboard/`](dashboard) is a small React + TypeScript app (Vite +
Recharts) that reads `vial_disposition.csv` directly and gives you an
interactive view of the same information as the static charts above, plus
a sortable, filterable per-vial table:

![Vial Tracking Dashboard: before/after tracking image, summary stat tiles, and interactive disposition/radius charts](docs/images/dashboard_screenshot.png)
...
It ships with the same sample data used throughout this README, so it runs
immediately. To point it at your own pipeline run instead:

```bash
python vial_analytics.py persistent_tracked_tracks.csv --out-dir analytics_output
cd dashboard
npm run sync-data   # copies analytics_output/ + docs/images/ into dashboard/public/
npm run dev
```

See [`dashboard/README.md`](dashboard/README.md) for the full breakdown —
project layout, the `sync-data` script, and how `npm run build` produces a
static site deployable to any static host. Its colors are pulled from the
exact same constants `vial_analytics.py` uses for its own charts
(`COLOR_GOOD`, `COLOR_WARNING`, etc.), so the two never disagree.
...
together gets caught too. A separate `dashboard` CI job type-checks and
builds the [web dashboard](#web-dashboard) on every push as well.
...
│   ├── test_tracker.py
│   └── test_analytics.py
├── dashboard/                      # React + TypeScript web dashboard (see Web dashboard)
├── .github/workflows/ci.yml       # Unit tests + full-pipeline smoke test + dashboard build, on every push
└── docs/
